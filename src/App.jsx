...
const COURSES = ["Medicine", "Natural Sciences", "Engineering", "Mathematics", "Economics", "Law", "PPE", "HSPS", "History", "English", "MML (Modern Languages)", "Computer Science", "PBS", "Classics"];
...
function TutorApply() {
  ...
  async function submit() {
    ...
    try { await postTutorApplication(f, tsToken); setSent(true); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  return (
    <section style={{ maxWidth: 720, margin: "0 auto", padding: "clamp(48px,7vw,80px) 40px clamp(64px,9vw,104px)" }}>
      ...
      <div style={{ display: "grid", gap: 22 }}>
        ...
        <div>
          <label style={L}>A-Levels achieved <span style={{ fontWeight: 400, color: MUTED }}>(as you'd like them shown)</span></label>
          <input style={field()} placeholder="A* Maths · A* Further Maths · A* Physics" value={f.alevels} onChange={(e) => setF({ ...f, alevels: e.target.value })} />
        </div>
...
function AdminPanel() {
  ...
  async function login() {
    ...
    try { const d = await callAdmin("list"); setTutors(d.tutors || []); setAuthed(true); }
